...
def get_config(cnf_file):
    ...
    if len(list(config.read(cnf_file, encoding='utf8'))) == 0:
        ...
        try:
            ...
            ShowMessage('Ошибка создания файла', 'Отсутствует конфигурационный файл.\nПри попытке создания файла ' + cnf_file + ' возникла ошибка:\n\n' + str(err), 0)
            sys.exit(-1)
    params = {}
...
def create_connection(db_file):
    try:
        ...
        ShowMessage('Ошибка подключения к БД', e, 0)
        sys.exit(-1)
...
def create_table(conn, create_table_sql):
    try:
        ...
        ShowMessage('Ошибка создания таблицы БД', e, 0)
        sys.exit(-1)
...
def query_table(conn, create_table_sql):
    try:
        ...
        ShowMessage('Ошибка получения данных из БД', e, 0)
        sys.exit(-1)
...
def save_to_html_begin(name_of_file):
    ...
    <table id='TableData'>
        <thead>
            <tr>
                <th>Сайт</th><th>Улица/район</th><th>Информация</th><th>Цена</th><th>Телефоны</th><th>Дата выгрузки</th>
            </tr>
        </thead>
        <tbody>""")
    return html_file
